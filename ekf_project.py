...
class EKF_Standard:
    """基礎未優化版：依賴 numpy 矩陣運算與求逆"""
    def __init__(self, x0, P0, Q, R):
        self.x = x0.reshape(4, 1)  # 狀態: [px, py, v, theta]^T
        self.P = P0                # 狀態共變異數矩陣 (4x4)
        self.Q = Q                 # 過程雜訊矩陣 (4x4)
        self.R = R                 # 觀測雜訊矩陣 (2x2)
        
        # 觀測矩陣 H (只有 px, py 是可觀測的)
        self.H = np.array([
            [1, 0, 0, 0],
            [0, 1, 0, 0]
        ])
    ...
    def update(self, z):
        ...
        # 2. 計算卡爾曼增益 K (系統效能殺手：矩陣求逆)
        S = self.H @ self.P @ self.H.T + self.R
        K = self.P @ self.H.T @ np.linalg.inv(S) # O(n^3) 的罪魁禍首
...
class EKF_FirmwareOptimized:
    """韌體優化版：純量化運算、對角矩陣簡化求逆、定點數預測模擬"""
    def __init__(self, x0, P0, Q, R):
        self.x = x0.flatten()      # 降為 1D Array，模擬 C 語言的一維陣列
        self.P = P0                # 共變異數矩陣
        self.Q = Q
        
        # 假設 GPS X/Y 雜訊獨立，只需存純量
        self.R_x = R[0, 0]
        self.R_y = R[1, 1]
    ...
    def predict_fixed_point(self, u, dt):
        ...
        # 雅可比與共變異數預測 (此處為模擬方便保留 numpy，真實 C 程式會展開為純量式)
...
# 第二部分：模擬環境設定與資料生成
# ==========================================
dt = 0.01          # IMU 更新週期 100Hz
time_steps = 2000  # 模擬 20 秒
